...
.. note::
   ...
   |st2| on Ubuntu ``22.04`` runs all services, actions and sensors using Python 3.
...
.. note::
    ...
    apt ``2.4.9`` discourages use of ``apt-key`` and the previous gpg file format.  The below instructions place the gpg key in the location and format expected by apt.
...
.. code-block:: bash
  ...
  sudo apt-get update
  sudo apt-get install -y crudini mongodb-org rabbitmq-server redis-server

If necessary, enable and start third party service MongoDB/RabbitMQ/Redis.

.. code-block:: bash

  sudo systemctl enable mongod.service rabbitmq-server.service redis-server.service
  sudo systemctl start mongod.service rabbitmq-server.service redis-server.service
...
SSL termination, and reverse-proxy st2auth, st2api and st2stream API endpoints. To set it up: install the
